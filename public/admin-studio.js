...
export function initStudio({request,action,saved,showImage}){
  ...
  document.querySelector('[data-delete-announcement]')?.addEventListener('click',async event=>{const form=event.currentTarget.closest('form');if(!await confirmAction('Delete this announcement from the homepage and admin editor?'))return;action(form,async()=>{const result=await request('/api/admin/announcement','DELETE',{version:Number(form.dataset.version)});saved(result.message);});});
  document.querySelector('.announcement-form')?.addEventListener('submit',event=>{event.preventDefault();const form=event.target;action(form,async()=>{const result=await request('/api/admin/announcement','PUT',{...Object.fromEntries(new FormData(form)),published:form.elements.published.checked,version:Number(form.dataset.version)});saved(result.message);});});
