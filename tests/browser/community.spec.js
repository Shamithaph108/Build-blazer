...
test('chat and recovery controls work on desktop and mobile',async({page})=>{
  ...
  expect(await page.locator('#announcement').evaluate(e=>e.compareDocumentPosition(document.querySelector('#home'))&Node.DOCUMENT_POSITION_PRECEDING)).toBeTruthy();
  await page.goto('/admin#announcements');await page.getByRole('button',{name:'Delete announcement',exact:true}).click();await page.locator('[data-confirm-accept]').click();await expect(page.locator('#studio-notice')).toContainText('Announcement deleted');await page.goto('/');await expect(page.locator('#announcement')).toHaveCount(0);
