...
**Vercel production email:** local `.env` values are not copied into Vercel. Add the variables above in **Project Settings → Environment Variables → Production**, then redeploy. `SMTP_HOST`, `SMTP_USER`, `SMTP_PASS` and a valid `MAIL_FROM` are required for replies. `SMTP_PORT` defaults to `587`; supported ports are `587` (STARTTLS) and `465` (TLS). `ADMIN_NOTIFY_EMAIL` is the separate destination for new-application notifications. Never enter your normal Google password as `SMTP_PASS` or commit any of these credentials.

If [Google App Passwords](https://myaccount.google.com/apppasswords) is unavailable, first confirm 2-Step Verification is enabled. Google also lists security-key-only verification, organization-managed accounts and Advanced Protection as possible restrictions ([official guidance](https://support.google.com/accounts/answer/185833)). Use a provider-issued SMTP credential from another authorized mailbox if this account cannot issue an App Password. Gmail OAuth requires a separate integration and is not implemented by this password-based SMTP transport.

Test the connection before sending. A `503` with `code: NOT_CONFIGURED` lists missing variable **names only**; `EAUTH` means the provider rejected authentication. Keep failed/queued records: after fixing configuration, use their **Send / retry email** control. Vercel runs sends inside requests; the local server's 15-second background timer does not run in a Vercel Function. Do not assume historical queued messages were delivered just because deployment succeeded. Do not retry a message already marked Sending until you check the sender mailbox.

**Vercel proxy headers:** the app automatically sets Express `trust proxy` to `1` when `VERCEL=1`, using Vercel's client `X-Forwarded-For`. The default `express-rate-limit` key generator, IPv6 grouping, limits and validation remain enabled. Other hosts still require explicit `TRUST_PROXY=1` only behind one trusted proxy. `GET /api/admin/notifications` and `GET /api/admin/submissions` require an authenticated session; the submissions endpoint returns the latest 500 records and is not publicly cached.
